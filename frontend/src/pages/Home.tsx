...
// ── Tilt card wrapper ─────────────────────────────────────────────────────────
function TiltCard({ children, className = '', strength = 10, style }: {
...
function RadarCanvas({ width, height }: { width: number; height: number }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      g.addColorStop(0, 'rgba(140,0,0,0.28)'); g.addColorStop(0.5, 'rgba(80,0,0,0.08)'); g.addColorStop(1, 'rgba(0,0,0,0)')
      ctx.fillStyle = g; ctx.fillRect(0, 0, width, height)
      pts.forEach(p => { p.x = ((p.x + p.vx) + width) % width; p.y = ((p.y + p.vy) + height) % height })
      for (let i = 0; i < pts.length; i++) for (let j = i + 1; j < pts.length; j++) {
        const d = Math.hypot(pts[i].x - pts[j].x, pts[i].y - pts[j].y)
        if (d < 52) { ctx.beginPath(); ctx.moveTo(pts[i].x, pts[i].y); ctx.lineTo(pts[j].x, pts[j].y); ctx.strokeStyle = `rgba(180,30,30,${(1 - d / 52) * 0.18})`; ctx.lineWidth = 0.4; ctx.stroke() }
      }
      ...
      ;[0.22, 0.42, 0.62, 0.82].forEach((frac, ri) => {
        ...
        ctx.strokeStyle = `rgba(220,38,38,${0.22 - ri * 0.04})`; ctx.lineWidth = ri === 0 ? 1.6 : 0.8
      ...
      sg.addColorStop(0, 'rgba(220,38,38,0.28)'); sg.addColorStop(1, 'rgba(220,38,38,0)')
      ...
      Array.from({ length: NODES }).forEach((_, i) => {
        ...
        ctx.fillStyle = `rgba(255,70,50,${0.6 + Math.sin(t * 1.3 + i) * 0.3})`; ctx.shadowColor = 'rgba(255,50,30,0.9)'; ctx.shadowBlur = 18; ctx.fill(); ctx.shadowBlur = 0
      })
      ;[0, 0.33, 0.66].forEach(off => {
        const phase = ((t * 0.5 + off) % 1)
        ctx.beginPath(); ctx.arc(cx, cy, 14 + phase * 55, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(220,38,38,${0.55 * (1 - phase)})`; ctx.lineWidth = 1.8; ctx.stroke()
      })
      ctx.beginPath(); ctx.arc(cx, cy, 18, 0, Math.PI * 2)
      const cg = ctx.createRadialGradient(cx, cy, 0, cx, cy, 18)
      cg.addColorStop(0, 'rgba(255,60,60,1)'); cg.addColorStop(0.4, 'rgba(220,38,38,0.95)'); cg.addColorStop(1, 'rgba(100,0,0,0.6)')
      ctx.fillStyle = cg; ctx.shadowColor = 'rgba(220,38,38,0.95)'; ctx.shadowBlur = 30; ctx.fill(); ctx.shadowBlur = 0
      t += 0.016
    ...
    <div className="relative select-none" style={{ width: W, height: H }}>
      ...
      <div className="absolute" style={{ left: cx, top: cy, transform: 'translate(-50%,-50%)' }}>
        <div className="relative" style={{ width: 96, height: 96 }}>
          ...
          <div className="absolute inset-[8%] rounded-full border border-red-700/40" />
          <div className="absolute inset-[18%] rounded-full bg-gradient-to-br from-red-900 to-black border-2 border-red-500/90 flex items-center justify-center"
            style={{ boxShadow: '0 0 40px rgba(220,38,38,0.7), 0 0 80px rgba(220,38,38,0.2), inset 0 1px 0 rgba(255,100,100,0.3)' }}>
            <svg viewBox="0 0 24 24" fill="none" className="w-9 h-9">
              <path d="M12 2L4 6v6c0 5.25 3.5 10.15 8 11.35C16.5 22.15 20 17.25 20 12V6l-8-4z"
                fill="rgba(220,38,38,0.4)" stroke="rgba(255,80,80,1)" strokeWidth="1.5" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
      </div>
      {CATS.map(cat => {
        ...
        return (
          <div key={cat.label} className="absolute" style={{ left: nx, top: ny, transform: 'translate(-50%,-50%)' }}>
            <TiltCard
              strength={8}
              className={`flex items-center gap-2 px-3 py-2 rounded-xl border border-gray-800/80 backdrop-blur-sm hover:border-red-900/50 transition-colors ${isLeft ? 'flex-row-reverse' : ''}`}
              style={{ background: 'rgba(10,10,14,0.96)', minWidth: 148, boxShadow: '0 8px 32px rgba(0,0,0,0.9), 0 0 0 1px rgba(255,255,255,0.04), inset 0 1px 0 rgba(255,255,255,0.05)' }}
            >
              <div className="h-8 w-8 rounded-lg bg-red-950/50 border border-red-900/40 flex items-center justify-center flex-shrink-0"
...
const FAQS = [
  ...
  { q: 'Can vindica remove data from breach databases?', a: "We can remove you from breach notification aggregators and people-search sites that index breach data. Original breach source databases cannot be deleted — but we minimize re-exposure by removing downstream aggregators." },
  { q: 'Do you offer refunds?', a: 'Yes. We offer a 14-day money-back guarantee on all paid plans. Contact support within 14 days of your first payment for a full refund.' },
]
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border border-gray-900/60 rounded-xl overflow-hidden"
      style={{ boxShadow: open ? '0 0 0 1px rgba(220,38,38,0.15), 0 8px 32px rgba(0,0,0,0.4)' : '0 4px 16px rgba(0,0,0,0.3)' }}>
      <button className="w-full flex items-center justify-between gap-4 px-6 py-4 text-left hover:bg-gray-900/20 transition-colors"
        onClick={() => setOpen(!open)}>
        <span className="text-sm font-semibold text-white">{q}</span>
        <ChevronDown className={`h-4 w-4 text-gray-500 flex-shrink-0 transition-transform ${open ? 'rotate-180 text-red-500' : ''}`} />
      </button>
      <AnimatePresence>
        {open && (
...
export function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#030305] text-white overflow-x-hidden">
      ...
      {/* ── Nav ──────────────────────────────────────────────────────────── */}
      <nav className={`sticky top-0 z-50 flex items-center justify-between px-8 lg:px-12 py-3.5 transition-all duration-300 ${scrolled ? 'bg-black/92 backdrop-blur-xl border-b border-white/5' : 'bg-transparent border-b border-gray-900/30'}`}
        style={scrolled ? { boxShadow: '0 1px 0 rgba(255,255,255,0.04), 0 8px 32px rgba(0,0,0,0.6)' } : undefined}>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to="/login" className="btn-red text-sm px-4 py-2 flex items-center gap-1.5"
            style={{ boxShadow: '0 0 20px rgba(220,38,38,0.4), 0 4px 12px rgba(0,0,0,0.4)' }}>
      ...
      <section className="max-w-screen-xl mx-auto px-8 lg:px-12 py-12 grid grid-cols-1 lg:grid-cols-[44%_56%] gap-8 items-center">
        <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="text-5xl lg:text-[3.5rem] font-extrabold leading-[1.08] tracking-tight mb-5">
            ...
            Is{' '}
            <span className="text-red-500" style={{ textShadow: '0 0 60px rgba(220,38,38,0.6), 0 0 120px rgba(220,38,38,0.2)' }}>
              Everywhere.
            </span><br />
            Take It Back.
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            <div className="relative flex-1 max-w-xs">
              ...
              <input className="input-field pl-9 text-sm w-full" placeholder="Enter your name or email…" />
            </div>
            <Link to="/login" className="btn-red flex items-center justify-center gap-2 whitespace-nowrap px-5 py-2.5"
              style={{ boxShadow: '0 0 24px rgba(220,38,38,0.5), 0 4px 16px rgba(0,0,0,0.4)' }}>
          ...
          <div className="grid grid-cols-3 gap-3 p-4 rounded-2xl"
            style={{ background: 'linear-gradient(135deg, rgba(15,15,20,0.95) 0%, rgba(8,8,12,0.95) 100%)', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 20px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.03), inset 0 1px 0 rgba(255,255,255,0.06)' }}>
            {[
      ...
      {/* ── Problem ───────────────────────────────────────────────────────── */}
      <section className="border-t border-white/5 py-20 bg-gradient-to-b from-transparent to-gray-950/30">
        <div className="max-w-screen-xl mx-auto px-8 lg:px-12">
          <motion.div className="text-center mb-14"
            ...
            <p className="text-gray-500 max-w-xl mx-auto text-sm leading-relaxed">Right now, hundreds of data brokers are buying, selling, and publishing your home address, phone number, family members, and financial history — without your consent.</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              ...
              <motion.div key={item.label}
                ...
                <TiltCard
                  className="h-full card-dark p-6 text-center hover:border-red-900/30 transition-colors rounded-2xl"
                  style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.03), inset 0 1px 0 rgba(255,255,255,0.05)' }}>
                  <div className="h-12 w-12 rounded-2xl bg-red-950/40 border border-red-900/30 flex items-center justify-center mx-auto mb-4"
                    style={{ boxShadow: '0 8px 24px rgba(220,38,38,0.2), inset 0 1px 0 rgba(255,100,100,0.1)' }}>
                    <item.icon className="h-5 w-5 text-red-500" />
                  </div>
                  <div className="text-3xl font-black text-red-500 mb-1" style={{ textShadow: '0 0 30px rgba(220,38,38,0.4)' }}>{item.stat}</div>
        ...
        <div className="max-w-screen-xl mx-auto px-8 lg:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            {[
              ...
              { step: '02', icon: Fingerprint, title: 'Exposure Map', desc: 'Get a complete Privacy Score and visual exposure map. See exactly where your data is, how severe each exposure is, and what is publicly available about you.', tags: ['Privacy Score', 'Severity rating', 'Visual map'] },
              { step: '03', icon: Shield, title: 'Auto Removals', desc: 'We auto-generate and send legally-binding GDPR/CCPA opt-out requests to every data broker. Track progress in real-time and get notified when removals are confirmed.', tags: ['GDPR requests', 'CCPA opt-outs', 'Auto-tracking'] },
            ].map((s, i) => (
              <motion.div key={s.step}
                initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12 }}>
                <TiltCard
                  className="relative h-full p-7 rounded-2xl border border-gray-800/60"
                  style={{ background: 'linear-gradient(145deg, rgba(14,14,18,0.98) 0%, rgba(8,8,11,0.98) 100%)', boxShadow: '0 24px 64px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.04), inset 0 1px 0 rgba(255,255,255,0.06)' }}>
                  <div className="flex items-center gap-3 mb-5">
                    <div className="h-10 w-10 rounded-xl bg-red-950/50 border border-red-900/40 flex items-center justify-center flex-shrink-0"
                      style={{ boxShadow: '0 0 24px rgba(220,38,38,0.25), inset 0 1px 0 rgba(255,100,100,0.15)' }}>
      ...
      {/* ── Platform Preview ─────────────────────────────────────────────── */}
      <section className="border-t border-white/5 py-20 bg-gradient-to-b from-gray-950/30 to-transparent overflow-hidden">
        <div className="max-w-screen-xl mx-auto px-8 lg:px-12">
          ...
          >
            <motion.div
              initial={{ rotateX: 20 }}
              whileInView={{ rotateX: 6 }}
              viewport={{ once: true }}
              transition={{ duration: 1, delay: 0.2 }}
              style={{ transformStyle: 'preserve-3d', transformOrigin: 'center bottom' }}
            >
              <div className="rounded-2xl border border-gray-800/60 overflow-hidden"
                style={{ boxShadow: '0 0 0 1px rgba(255,255,255,0.05), 0 40px 80px rgba(0,0,0,0.8), 0 80px 160px rgba(220,38,38,0.06)' }}>
                {/* Window chrome */}
                <div className="flex items-center gap-3 px-5 py-3 bg-gray-950 border-b border-gray-900/60"
                  style={{ background: 'linear-gradient(180deg, rgba(20,20,26,1) 0%, rgba(12,12,16,1) 100%)' }}>
                  ...
                  <div className="flex-1 mx-4">
                    <div className="h-5 rounded bg-gray-900/80 flex items-center px-3 gap-2 max-w-xs mx-auto border border-gray-800/60">
                      <Lock className="h-2.5 w-2.5 text-green-500/70" />
                      <span className="text-[10px] text-gray-500">vindica.me/app</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#030305] p-5 grid grid-cols-12 gap-4">
                  ...
                  <div className="col-span-2 space-y-0.5">
                    {['Scan', 'Removals', 'Reports', 'Monitor', 'Track', 'Settings'].map((item, i) => (
                      <div key={item} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold ${i === 0 ? 'text-red-400 border border-red-900/30' : 'text-gray-600 hover:text-gray-400'}`}
                        style={i === 0 ? { background: 'linear-gradient(135deg, rgba(80,0,0,0.4) 0%, rgba(40,0,0,0.3) 100%)', boxShadow: '0 4px 12px rgba(220,38,38,0.1)' } : undefined}>
                  ...
                  <div className="col-span-10 space-y-4">
                    <div className="grid grid-cols-4 gap-3">
                      {[
                        ...
                        <div key={k.label} className="rounded-xl border border-gray-900/60 p-3"
                          style={{ background: 'linear-gradient(145deg, rgba(15,15,20,0.9) 0%, rgba(8,8,12,0.9) 100%)', boxShadow: '0 8px 24px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.04)' }}>
                          ...
                          <div className={`text-xl font-black ${k.red ? 'text-red-500' : 'text-white'}`}
                            style={k.red ? { textShadow: '0 0 16px rgba(220,38,38,0.6)' } : undefined}>{k.val}</div>
                          <div className={`text-[9px] ${k.red ? 'text-red-400' : 'text-gray-600'}`}>{k.sub}</div>
                        </div>
                      ))}
                    </div>

                    <div className="rounded-xl border border-gray-900/60 overflow-hidden"
                      style={{ background: 'linear-gradient(145deg, rgba(12,12,16,0.9) 0%, rgba(6,6,9,0.9) 100%)' }}>
                      <div className="px-4 py-2.5 border-b border-gray-900/60 flex items-center gap-2"
                        style={{ background: 'linear-gradient(180deg, rgba(20,20,26,0.8) 0%, transparent 100%)' }}>
                        <ShieldAlert className="h-3.5 w-3.5 text-red-500" />
                        <span className="text-[11px] font-bold text-white">Active Breach Records</span>
                        <span className="ml-auto text-[10px] text-red-400 bg-red-950/40 border border-red-900/40 px-2 py-0.5 rounded-full">6 found</span>
                      </div>
                      <div className="divide-y divide-gray-900/40">
                        {[
                          ...
                          <div key={r.src} className="flex items-center gap-3 px-4 py-2 hover:bg-white/[0.02] transition-colors">
                            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${r.sev === 'critical' ? 'bg-red-950/70 text-red-400 border border-red-900/50' : r.sev === 'high' ? 'bg-red-950/50 text-red-400 border border-red-900/30' : 'bg-gray-900/70 text-gray-500 border border-gray-800/60'}`}>{r.sev.toUpperCase()}</span>
                            <span className="text-[10px] font-semibold text-white w-24">{r.src}</span>
                            <span className="text-[9px] text-gray-600">{r.fields}</span>
                            <span className="ml-auto text-[9px] text-red-500 border border-red-900/30 px-2 py-0.5 rounded cursor-pointer hover:bg-red-950/20 transition-colors">Remove →</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {/* Ground reflection */}
          <div className="relative h-16 -mt-2 overflow-hidden" style={{ mask: 'linear-gradient(to bottom, white, transparent)' }}>
            <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(220,38,38,0.04), transparent)' }} />
          </div>
      ...
      {/* ── Broker Coverage ───────────────────────────────────────────────── */}
      <section id="data-brokers" className="border-t border-white/5 py-16">
        <div className="max-w-screen-xl mx-auto px-8 lg:px-12">
      ...
      {/* ── Testimonials ─────────────────────────────────────────────────── */}
      <section className="border-t border-white/5 py-20 bg-gradient-to-b from-transparent to-gray-950/20">
        <div className="max-w-screen-xl mx-auto px-8 lg:px-12">
          <motion.div className="text-center mb-12"
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { name: 'Sarah K.', role: 'Marketing Executive', loc: 'New York, NY', avatar: 'https://i.pravatar.cc/64?img=47', text: 'Within 48 hours, vindica found my address, phone number, and family members on 23 different data broker sites and sent removal requests automatically.', stat: '23 sources removed' },
              { name: 'James T.', role: 'Attorney', loc: 'Chicago, IL', avatar: 'https://i.pravatar.cc/64?img=12', text: "As a lawyer, my personal information being publicly accessible was a genuine safety risk. vindica's automated GDPR opt-out system is legally sound and remarkably effective. Score went from 31 to 74.", stat: 'Score +43 pts' },
              { name: 'Priya M.', role: 'Software Engineer', loc: 'San Francisco, CA', avatar: 'https://i.pravatar.cc/64?img=58', text: "I just entered my email and watched vindica do everything — scanned, scored, sent removals, and gave me a detailed breach report. Incredibly well designed.", stat: '47 opt-outs sent' },
            ].map((t, i) => (
              <motion.div key={t.name}
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                <TiltCard
                  className="h-full p-6 rounded-2xl border border-gray-800/50 hover:border-red-900/25 transition-colors flex flex-col"
                  style={{ background: 'linear-gradient(145deg, rgba(14,14,18,0.98) 0%, rgba(8,8,11,0.98) 100%)', boxShadow: '0 24px 64px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.03), inset 0 1px 0 rgba(255,255,255,0.05)' }}>
                  ...
                  <div className="pt-4 border-t border-gray-900/60 flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-bold text-red-400 bg-red-950/30 border border-red-900/30 px-2.5 py-1 rounded-full"
                      style={{ boxShadow: '0 0 12px rgba(220,38,38,0.1)' }}>{t.stat}</span>
                  </div>
                </TiltCard>
              </motion.div>
            ))}
      ...
      <section id="pricing" className="border-t border-white/5 py-20">
        <div className="max-w-screen-xl mx-auto px-8 lg:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {[
              { tier: 'Free', price: '$0', period: 'forever', desc: 'Get started and see your exposure.', features: ['One-time exposure scan','Privacy Score','Top 5 broker listings','Breach report preview','Basic email alerts'], cta: 'Start Free', featured: false },
              { tier: 'Pro', price: '$9', period: '/month', desc: 'Full control for privacy-conscious individuals.', features: ['Continuous monitoring','196+ source scans','Automated opt-out requests','Dark web monitoring','GDPR/CCPA legal requests','Monthly PDF reports','Priority support'], cta: 'Get Pro', featured: true, badge: 'Most Popular' },
              { tier: 'Business', price: '$49', period: '/month', desc: 'For executives, legal teams, and families.', features: ['Everything in Pro','Up to 5 profiles','Executive threat reports','Honey token monitoring','API access','Custom compliance reports','Dedicated account manager'], cta: 'Contact Sales', featured: false },
            ].map((plan, i) => (
              <motion.div key={plan.tier}
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                <TiltCard
                  strength={plan.featured ? 8 : 6}
                  className="relative h-full rounded-2xl border p-7 flex flex-col"
                  style={{
                    borderColor: plan.featured ? 'rgba(220,38,38,0.5)' : 'rgba(255,255,255,0.06)',
                    background: plan.featured
                      ? 'linear-gradient(145deg, rgba(40,4,4,0.6) 0%, rgba(14,14,18,0.98) 40%, rgba(8,8,11,0.98) 100%)'
                      : 'linear-gradient(145deg, rgba(14,14,18,0.98) 0%, rgba(8,8,11,0.98) 100%)',
                    boxShadow: plan.featured
                      ? '0 0 0 1px rgba(220,38,38,0.3), 0 32px 80px rgba(0,0,0,0.6), 0 0 60px rgba(220,38,38,0.08), inset 0 1px 0 rgba(255,80,80,0.08)'
                      : '0 24px 64px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.03), inset 0 1px 0 rgba(255,255,255,0.04)',
                  }}>
                  {plan.badge && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                      <span className="text-[10px] font-bold text-white bg-red-600 px-3 py-1 rounded-full"
                        style={{ boxShadow: '0 4px 16px rgba(220,38,38,0.5)' }}>{plan.badge}</span>
                    </div>
                  )}
                  <div className="mb-5">
                    ...
                    <div className="flex items-end gap-1 mb-1">
                      <span className="text-4xl font-black text-white"
                        style={plan.featured ? { textShadow: '0 0 30px rgba(220,38,38,0.3)' } : undefined}>{plan.price}</span>
                  ...
                  <Link to="/login"
                    className={`w-full text-center py-2.5 rounded-xl text-sm font-semibold transition-all ${plan.featured ? 'bg-red-600 hover:bg-red-700 text-white' : 'border border-gray-700 hover:border-gray-600 text-gray-300 hover:text-white'}`}
                    style={plan.featured ? { boxShadow: '0 0 24px rgba(220,38,38,0.4), 0 4px 16px rgba(0,0,0,0.4)' } : undefined}>
                    {plan.cta}
                  </Link>
                </TiltCard>
              </motion.div>
      ...
      {/* ── Trust ────────────────────────────────────────────────────────── */}
      <section className="border-t border-white/5 py-14 bg-gray-950/30">
        <div className="max-w-screen-xl mx-auto px-8 lg:px-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <motion.div key={item.title}
                ...
                <TiltCard className="flex gap-3 items-start" strength={6}>
                  <div className="h-9 w-9 rounded-xl bg-red-950/30 border border-red-900/20 flex items-center justify-center flex-shrink-0"
                    style={{ boxShadow: '0 0 16px rgba(220,38,38,0.1), inset 0 1px 0 rgba(255,100,100,0.08)' }}>
      ...
      <section className="border-t border-white/5 py-24 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 50%, rgba(120,0,0,0.15) 0%, transparent 70%)' }} />
          <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 40% 30% at 50% 60%, rgba(80,0,0,0.08) 0%, transparent 70%)' }} />
        </div>
        <motion.div className="relative max-w-2xl mx-auto px-8 text-center"
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
            <TiltCard strength={6}>
              <Link to="/login"
                className="btn-red inline-flex items-center gap-2 px-8 py-4 text-base font-bold rounded-xl"
                style={{ boxShadow: '0 0 40px rgba(220,38,38,0.5), 0 8px 32px rgba(0,0,0,0.4)' }}>
            ...
            <TiltCard strength={4}>
              <Link to="/login"
                className="inline-flex items-center gap-2 px-6 py-4 text-sm font-semibold text-gray-400 hover:text-white border border-gray-800 hover:border-gray-700 rounded-xl transition-colors"
            ...
            <div className="col-span-2">
              ...
              <div className="flex gap-3">
                {[Twitter, Github, Linkedin, Mail].map((Icon, i) => (
                  <TiltCard key={i} strength={12}>
                    <a href="#" className="h-8 w-8 rounded-lg border border-gray-900 flex items-center justify-center text-gray-700 hover:text-gray-400 hover:border-gray-700 transition-colors"
                      style={{ boxShadow: '0 4px 12px rgba(0,0,0,0.4)' }}>
            ...
            {[
              ...
              <div key={col.title}>
                ...
                <ul className="space-y-2">
                  {col.links.map(link => (
                    <li key={link}><a href="#" className="text-xs text-gray-700 hover:text-gray-400 transition-colors">{link}</a></li>
                  ))}
                </ul>
              </div>
            ))}
