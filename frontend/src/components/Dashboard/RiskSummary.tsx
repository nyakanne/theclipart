...
export function RiskSummary({ result }: { result: ScanResult }) {
  ...
  const riskColor = result.risk_score >= 75 ? 'text-red-400'
    : result.risk_score >= 40 ? 'text-red-200' : 'text-gray-200'
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
      ...
      />
      <StatCard
        icon={<Database className="h-6 w-6 text-red-300" />}
      ...
      />
      <StatCard
        icon={<Globe className="h-6 w-6 text-red-300" />}
      ...
      />
      <StatCard
        icon={<Zap className="h-6 w-6 text-red-300" />}
        label="Token Hits"
