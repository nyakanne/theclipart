...
export function MobileTabBar() {
  ...
  return (
    <nav
      ...
      className="mobile-tabbar fixed inset-x-0 bottom-0 z-50 border-t border-white/10 bg-ink-900/95 px-2 pb-[calc(env(safe-area-inset-bottom)+0.45rem)] pt-2 shadow-[0_-18px_44px_rgba(0,0,0,0.48)] backdrop-blur-xl lg:hidden"
    >
          ...
          return (
            <Link
              ...
              className={clsx(
                ...
                active
                  ? 'bg-gold-500/10 text-gold-400 shadow-[inset_0_0_0_1px_rgba(212,175,55,0.18)]'
                  : 'text-gray-500 hover:bg-white/[0.04] hover:text-gray-200'
              )}
            >
              <Icon className={clsx('mb-1 h-5 w-5', active ? 'text-gold-400' : 'text-gray-500')} />
              <span>{label}</span>
            </Link>
          )
        })}
