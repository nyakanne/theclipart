...
Runs with SQLite, no Celery, no Redis, no AWS. Returns realistic mock data.

Usage:
  DEMO_MODE=true python -m uvicorn backend.demo_main:app --port 8000 --reload
  or from repo root:
  cd backend && DEMO_MODE=true python -m uvicorn demo_main:app --port 8000 --reload
"""
import asyncio
import json
import os
...
BREACH_SOURCES = [
    ...
    {"name": "HaveIBeenPwned", "year": "2019", "fields": ["Email addresses"], "count": 21_000_000, "severity": "low"},
...
BROKERS = [
    ...
    {"name": "Epsilon", "url": "https://epsilon.com", "opt_out_url": "https://us.epsilon.com/privacy-policy", "category": "Data Broker"},
    ...
    {"name": "Pipl", "url": "https://pipl.com", "opt_out_url": "https://pipl.com/personal-information-removal-request", "category": "People Search"},
    {"name": "PublicRecordsNow", "url": "https://publicrecordsnow.com", "opt_out_url": "https://www.publicrecordsnow.com/static/view/optout", "category": "Public Records"},
...
def _mock_scan_result(query: dict) -> dict:
    ...
    # Deterministic-ish based on input length
    ...
    breaches = [
        {
            ...
            "description": f"In {b['year']}, {b['name']} suffered a data breach exposing {b['count']:,} records including {', '.join(b['fields'][:2])}.",
            "verified": True,
        }
    ...
    compliance = {
        ...
        "recommendations": [
            "Immediately opt out from all data broker listings found.",
...
async def create_scan(body: ScanRequest):
    ...
    return {"scan_id": scan_id, "status": "scanning", "progress": 0.0,
            "current_stage": "Checking breach databases…",
...
async def queue_optouts(scan_id: str):
    ...
    return {
        "queued": len(listings),
        "message": f"DEMO: {len(listings)} opt-out links queued. In production, automated DSAR requests would be submitted.",
        "note": "This is demo mode — no actual opt-out submissions are made.",
    }
